...
- Add support for diagram appearance customization
...
- https://github.com/eclipse-sirius/sirius-web/issues/6887[#6887] [core] Improve the consitency of the variables used.
...
- https://github.com/eclipse-sirius/sirius-web/issues/6868[#6868] [core] The classes `Module`, `Window`, `Failure`, `IResult`, `Success`, and `AbstractValidatingAggregateRoot` has been moved to the new module `sirius-web-core-domain`.
...
- [view] Default tools are now provided only for low-code studios.
`DefaultToolsFactory` has been renamed to `StudioDefaultToolsFactory`.
The following factory methods have been removed: `createDefaultSourceEdgeReconnectionTool`, `createDefaultTargetEdgeReconnectionTool`, `createDefaultBeginLabelEditTool`, `createDefaultCenterLabelEditTool`, `createDefaultEndLabelEditTool`, `createDefaultHideAllChildrenNodeTool`, `createDefaultRevealAllChildrenNodeTool`, `createDefaultResetAllChildrenVisibilityModifiersNodeTool`, and `createDefaultRevealChildrenWithValueNodeTool`.
`Reset All Children Visibility` and `Reveal Children With Value` tools used by Flow DSL have been removed.
`DefaultMultiSelectionVisibilityToolsFactory` has been removed from `sirius-components-view-diagram-edit`.
Programmatic DSLs that relied on these factories must define the tools they expose in their own description providers.
- https://github.com/eclipse-sirius/sirius-web/issues/5928[#5928] [workbench] Contributed views should now use the new `ViewAccordion` component with its two subcomponents:
* `ViewAccordionToolbar` if the view has a toolbar
* `ViewAccordionContent` for the content of the view.
You can check Sirius Web's views for concrete examples.
`ViewAccordion` gives its content a definite height, so the content of a view must not scroll the view itself: it has to declare its own `overflow` on the element which is expected to scroll.
Using the `ViewAccordion` will merge the title bar and the toolbar (if any) of the view into a single header bar.
- https://github.com/eclipse-sirius/sirius-web/issues/5928[#5928] [workbench] The workbench header bar (merge of the title + toolbar) now has a grayish background.
This color can be changed with through the theme palette `view.background`.
Defining this color may be required.
- [cleanup] Rename test profile to e2e.
`sirius-web/backend/sirius-web/docker-compose.yml` now activates the `e2e` profile instead of the `test` profile.
Existing services requiring the `test` profile have been updated.
- https://github.com/eclipse-sirius/sirius-web/issues/6868[#6868] [libraries] Classes from the library domain such as `Library`, `ILibrarySearchService`, `ILibraryCreationService` or `LibraryCreatedEvent` have been moved to the new module `sirius-web-library-domain`.
Imports of these classes must be updated to:
...
- https://github.com/eclipse-sirius/sirius-web/issues/6740[#6740] [sirius-web] Add support for required libraries in templates.
A new `List<ProjectTemplateRequiredLibrary>` argument is now required to create a `ProjectTemplate`.
- https://github.com/eclipse-sirius/sirius-web/issues/6868[#6868] [images] Classes from the image domain such as `Image`, `IImageSearchService`, `IImageCreationService` or `ImageCreatedEvent` have been moved to the new module `sirius-web-images-domain`.
Imports of these classes must be updated to:
...
- https://github.com/eclipse-sirius/sirius-web/issues/6194[#6194] [core] Remove the field `ErrorPayload#message` in favor of `ErrorPayload#messages`.
- https://github.com/eclipse-sirius/sirius-web/issues/6868[#6868] [sirius-web] The project-image domain has moved to the dedicated Maven module `sirius-web-projects-images-domain`.
...
- https://github.com/eclipse-sirius/sirius-web/issues/6868[#6868] [sirius-web] The project semantic data domain has moved to the dedicated Maven module `sirius-web-projects-semanticdata-domain`.
...
- https://github.com/eclipse-sirius/sirius-web/issues/6942[#6942] [core] Remove the deprecated types `IObjectService` and `ObjectService` since they have been replaced a long time ago by smaller services such as `IIdentityService`, `IObjectSearchService`, `ILabelService`, etc.
- https://github.com/eclipse-sirius/sirius-web/issues/6868[#6868] [images] Reduce the dependency of the images bounded context on `sirius-web-domain` by adding a dedicated configuration module, `sirius-web-images-infrastructure`, for its internationalization support.
- https://github.com/eclipse-sirius/sirius-web/issues/2770[#2770] [diagram] Remove `findNodeIndex` from `@eclipse-sirius/sirius-components-diagrams`.
Custom node layout handlers should use `getNodeBorderWidth(node.data.style)` instead of inspecting hidden rendered nodes.
Consumers that still need a node index can use `nodes.findIndex((node) => node.id === nodeId)`.
- https://github.com/eclipse-sirius/sirius-web/issues/6868[#6868] [sirius-web] Reduce the dependency of the project images bounded context on `sirius-web-domain` by adding a dedicated configuration module, `sirius-web-projects-images-infrastructure`, for its internationalization support.
- https://github.com/eclipse-sirius/sirius-web/issues/6931[#6931] Remove `connectorTools` field from GraphQL type `DiagramDescription` and related DTO, use `connectorPalette` instead, `IConnectorToolsProvider` & `GetConnectorToolsEventHandler` have been removed too.
- https://github.com/eclipse-sirius/sirius-web/issues/6976[#6976] [form] Remove the reference widget clear handler from the refresh mechanism.
Therefore, it isn't possible anymore to specify the clear handler as a lambda in the `ReferenceWidgetDescription`.
Instead, a new API has been added: `IReferenceWidgetClearHandler`, a candidate to handle the clear event; and `IDefaultReferenceWidgetClearHandler` the default implementation used when no candidate is able to handle the clear event.
An implementation of `IDefaultReferenceWidgetClearHandler`, `ReferenceWidgetDefaultClearHandler`, is by default provided by Sirius Web, but it can be replaced by a custom implementation using the Spring priority annotation mechanism.


=== Dependency update

- [releng] Switch to https://github.com/spring-projects/spring-boot/releases/tag/v4.1.1[Spring Boot 4.1.1].
- [releng] Switch to https://github.com/testing-library/user-event/releases/tag/v14.6.7[@testing-library/user-event 14.6.7].
- [releng] Switch to https://github.com/testing-library/react-testing-library/releases/tag/v16.3.3[@testing-library/react 16.3.3].
- [releng] Switch to https://github.com/vercel/turborepo/releases#release-v2.10.12[turbo 2.10.12].
- [releng] Switch to https://github.com/vitejs/vite/releases/tag/v8.3.0[Vite 8.3.0] and https://github.com/vitejs/vite-plugin-react/releases/tag/plugin-react%406.1.1[@vitejs/plugin-react 6.1.1]
- [releng] Switch to https://github.com/nodejs/node/releases/tag/v24.20.0[node.js 24.20.0] and https://docs.npmjs.com/cli/v11/using-npm/changelog#11190-2026-07-28[npm 11.19.0] for our development environment.


=== Bug fixes

- https://github.com/eclipse-sirius/sirius-web/issues/3914[#3914] [diagram] Allow icon label nodes to be moved inside free-form containers.
- https://github.com/eclipse-sirius/sirius-web/issues/3712[#3712] [diagram] Render child and border nodes declared under a border node.
- https://github.com/eclipse-sirius/sirius-web/issues/6939[#6939] [diagram] The available `self` variable for the `precondition` of the `connector palette` is now set to the `semantic source` of connection (it was previously the target).
- https://github.com/eclipse-sirius/sirius-web/issues/2770[#2770] [diagram] Prevent React Flow handle warnings while computing the initial diagram layout.


=== New Features

- https://github.com/eclipse-sirius/sirius-web/issues/6810[#6810] [diagram] Add a new toolbar menu button to manage diagram elements selection
- https://github.com/eclipse-sirius/sirius-web/issues/6952[#6952] [diagram] Add `IDiagramToolbarFilterSelectionProvider` to add options to the manage diagram selection diagram toolbar tool.
- https://github.com/eclipse-sirius/sirius-web/issues/6960[#6960] [diagram] Add `IDiagramToolbarFilterSelectionHandler` to add handlers to the manage diagram selection diagram toolbar tool.


=== Improvements

- https://github.com/eclipse-sirius/sirius-web/issues/6047[#6047] [validation] Perform the refresh of the validation representation outside of its representation event processor.
A new provisional method `IValidationEventProcessor#update` is available to update the state of the validation event processor and publish a new validation representation to subscribers.
- https://github.com/eclipse-sirius/sirius-web/issues/6868[#6868] [core] Add a new module named `sirius-web-core-domain` to extract the core reusable parts of `sirius-web-domain`.
The classes `Module`, `Window`, `Failure`, `IResult`, `Success`, and `AbstractValidatingAggregateRoot` has been moved to this new module.
- https://github.com/eclipse-sirius/sirius-web/issues/6844[#6844] [view] Expose available variables in the Javadoc of generated View DSL builders.
- https://github.com/eclipse-sirius/sirius-web/issues/5928[#5928] [workbench] All sirius-web views in the workbench are now collapsible.
- https://github.com/eclipse-sirius/sirius-web/issues/6887[#6887] [core] Improve the consistency of the variables used.
For that, some calls to `VariableManager.SELF` have thus been replaced by `RepresentationVariables.SELF.name()` and calls to `IEditingContext.EDITING_CONTEXT` have been replaced by `CoreVariables.EDITING_CONTEXT.name()`.
- https://github.com/eclipse-sirius/sirius-web/issues/6887[#6887] [deck] Specifiers customizing the creation, deletion, edition and drop of cards will now be able to leverage the `editingContext` variable.
- [doc] Update the backend documentation and add a first schema of the lifecycle of the backend with the creation of a project.
- https://github.com/eclipse-sirius/sirius-web/issues/6868[#6868] [libraries] Extract the domain parts of the libraries bounded-context into the `sirius-web-library-domain` module.
- https://github.com/eclipse-sirius/sirius-web/issues/6740[#6740] [sirius-web] Add support for required libraries in templates.
Templates can now define required libraries (specifying their namespace, name, and version).
These libraries are selected by default in the project creation's library table, and cannot be unselected.
- [releng] Run end-to-end tests on master and only on relevant pull requests.
The `ci: run e2e` label can be added to a pull request to force end-to-end test execution.
- https://github.com/eclipse-sirius/sirius-web/issues/6868[#6868] [images] Extract the domain parts of the image bounded context into the `sirius-web-images-domain` module.
- [sirius-web] `IProjectSemanticDataSearchService` now supports retrieving a `ProjectSemanticData` by its own identifier through `findById(UUID projectSemanticDataId)`.
- https://github.com/eclipse-sirius/sirius-web/issues/6939[#6939] [diagram] Added more `AQL variables` available when computing the precondition of a tool in a palette, this includes for a `connector palette` : `environment`, `diagramServices`, `selectedNode`, `selectedEdge` and for a `diagram element palette` : `environment`, `diagramServices`.
- [diagram] Extract the appearance logic of nodes and edges in a dedicated service.
- https://github.com/eclipse-sirius/sirius-web/issues/6824[#6824] [form] Add RichTextEditor link insertion and link edition.
- https://github.com/eclipse-sirius/sirius-web/issues/6806[#6806] [diagram] Retrieve the list of `FilterSelectionMenuItems` (menu items shown on the manage selection tool from the diagram toolbar) from `filterSelectionMenuItems` field in GraphQL type `DiagramToolbar`.
- [starters] Extract the logic to check if a project has the FLOW nature into a dedicated predicate.
- https://github.com/eclipse-sirius/sirius-web/issues/6957[#6957] [diagram] Add translations support to manage selection diagram toolbar tool
- https://github.com/eclipse-sirius/sirius-web/issues/6808[#6808] [diagram] Invoke manage selection diagram panel tool from the backend
- https://github.com/eclipse-sirius/sirius-web/issues/6976[#6976] [form] Remove reference widget clear handler from refresh mechanism.
...
- https://github.com/eclipse-sirius/sirius-web/issues/6813[#6813] [diagram] The GraphQL field `DiagramDescription#connectorTools(...)` is now deprecated and will be removed soon in favor of `DiagramDescription#connectorPalette(...)``.


=== Breaking changes

- [releng] Frontend packages published from `packages/*/frontend/` do not provide UMD bundles anymore.
...
- [sirius-web] Remove the file `reset.css` from the Sirius Web sample project and rely entirely on MUI's `CssBaseline`.
...
- https://github.com/eclipse-sirius/sirius-web/issues/6635[#6635] [sirius-web] Add a dedicated package for markdown edition and rendering.
...
- https://github.com/eclipse-sirius/sirius-web/issues/6674[#6674] [palette] An overridden tool contribution has to return null or an empty fragment if the `searchedValue` does not match the label of the overridden tool, we can't guess what label will be used in external contributions.
- https://github.com/eclipse-sirius/sirius-web/issues/6799[#6799] [palette] The DTO `Palette`, `ITool`, `IPaletteEntry`, `PaletteDivider` & `ToolSection` have been moved to new maven project `sirius-components-palette`.
...
- https://github.com/eclipse-sirius/sirius-web/issues/6788[#6788] [tree] The `TreeDescriptionPaletteDataFetcher` does not retrieve all tools from the implementations of `ITreeItemPaletteProvider`, you'll need a custom `ITreeItemPaletteProvider` in order to add your own tool.
- https://github.com/eclipse-sirius/sirius-web/issues/6815[#6815] [tree] In case a tree item palette provider is available for a representation, the `ITreeItemContextMenuEntryProvider` contributions will be ignored.
...
- https://github.com/eclipse-sirius/sirius-web/issues/6835[#6835] [tree] `withImpactAnalysis` attribute has been removed from `GQLFetchTreeItemContextMenuEntry` type, it's available on `GQLSingleClickTreeItemContextMenuEntry`.
- https://github.com/eclipse-sirius/sirius-web/issues/6866[#6866] [tree] When using `CustomTreeItemContextMenuEntry` with a `contributionId` reused from sirius web, you will now also need to set the `labelExpression` in order for the label to render correctly on the frontend contribution.
...
- [releng] Remove usage of the development dependency `rollup-plugin-peer-deps-external` in favor of native Vite based solution
- [releng] Update the version of `turbo` used for the frontend from 2.4.4 to 2.10.5.
- [releng] Update `vite` from 8.0.14 to 8.1.5.
- [releng] Switch to https://github.com/spring-projects/spring-boot/releases/tag/v4.1.0[Spring Boot 4.1.0]
- [releng] Update `playwright` from 1.57.0 to 1.62.0


=== Bug fixes

- https://github.com/eclipse-sirius/sirius-web/issues/6662[#6662] [diagram] Center the position of the creation handles relative to their node
- https://github.com/eclipse-sirius/sirius-web/issues/6704[#6704] [diagram] Add a translation for the warning notification that no edges are available.
- https://github.com/eclipse-sirius/sirius-web/issues/5706[#5706] [diagram] Fix post tool selection not set correctly on diagrams
- https://github.com/eclipse-sirius/sirius-web/issues/5369[#5369] [form] Prevent existing values from being added again when selecting a new value in a multi-valued reference widget.
- https://github.com/eclipse-sirius/sirius-web/issues/6694[#6694] [diagram] Fix incorrect bending point handling for non-Manhattan edges during layout
- https://github.com/eclipse-sirius/sirius-web/issues/6667[#6667] [diagram] Preserve the layout of child nodes while resizing a free-form node.
- https://github.com/eclipse-sirius/sirius-web/issues/6640[#6640] [diagram] Apply `resizedByUser` flag to list children.
- https://github.com/eclipse-sirius/sirius-web/issues/6674[#6674] [palette] Add overridden contribution to palette search results.
...
- https://github.com/eclipse-sirius/sirius-web/issues/6746[#6746] [diagram] Fix an issue where a creation handle could stay highlighted after a connection
- https://github.com/eclipse-sirius/sirius-web/issues/6732[#6732] [diagram] Fix node decorators not showing on hover.
- https://github.com/eclipse-sirius/sirius-web/issues/6759[#6759] [palette] Fix palette tool search sometimes dropping matching elements
- https://github.com/eclipse-sirius/sirius-web/issues/6782[#6782] [diagram] Fix an issue where quicktools contributed from `IGroupPaletteToolsProvider` were displayed after the ones from the view model.
- https://github.com/eclipse-sirius/sirius-web/issues/6769[#6769] [diagram] Resize free-form parent nodes to contain child nodes moved partially outside their bounds.
- https://github.com/eclipse-sirius/sirius-web/issues/6770[#6770] [diagram] Prevent hidden child nodes from affecting the resize of their free-form parent.
- https://github.com/eclipse-sirius/sirius-web/issues/6801[#6801] [diagram] Improve lasso selection to only include a single node as "last selected" instead of several ones when they are aligned.
- https://github.com/eclipse-sirius/sirius-web/issues/6835[#6835] [tree] Fix an issue where tree item tools declared from a view dsl were not executed correctly
- https://github.com/eclipse-sirius/sirius-web/issues/6847[#6847] [sirius-web] Fix invalid studio when created from template by setting a default name to generated color palette.
- https://github.com/eclipse-sirius/sirius-web/issues/6846[#6846] [diagram] Fix "Last Selected Element" that was not correctly set after a diagram refresh.
- https://github.com/eclipse-sirius/sirius-web/issues/6849[#6849] [diagram] Fix "Last Selected Element" that was not correctly set after a tool redefined the selection.
- https://github.com/eclipse-sirius/sirius-web/issues/6855[#6855] [tree] Fix an issue where a tree item tooltip could stay displayed while the palette was opened.
- https://github.com/eclipse-sirius/sirius-web/issues/6851[#6851] [diagram] Fix global selection so it targets the last selected element.
- https://github.com/eclipse-sirius/sirius-web/issues/6857[#6857] [diagram] Fix an issue where a tool could be executed in a readonly diagram using a tool keyboard shortcut.
- https://github.com/eclipse-sirius/sirius-web/issues/6871[#6871] [diagram] Fix an issue where no remaining node was marked as last selected after unselecting multiple nodes with the lasso.
- https://github.com/eclipse-sirius/sirius-web/issues/6893[#6893] [workbench] Fix an issue that prevented the workbench from being shared.
- https://github.com/eclipse-sirius/sirius-web/issues/6895[#6895] [tree] Fix an issue where the impact analysis modal was not opening after updating a library with the tree item palette.
- https://github.com/eclipse-sirius/sirius-web/issues/6906[#6906] [sirius-web] Remove stray semicolon from the library view.

=== New Features

- https://github.com/eclipse-sirius/sirius-web/issues/6540[#6540] [diagram] Use the palette to render connector tools
- https://github.com/eclipse-sirius/sirius-web/issues/6813[#6813] [diagram] Retrieve an entire palette for the connector from the backend.
For that, a new GraphQl field `DiagramDescription#connectorPalette(sourceDiagramElementId: ID!, targetDiagramElementId: ID!): Palette!` has been added to retrieve the palette of the connector tool.
- https://github.com/eclipse-sirius/sirius-web/issues/6788[#6788] [tree] Introduce GraphQL `palette` field on `TreeDescription` type, added both type `FetchTreeItemTool` & `SingleClickTreeItemTool` that implements `ITool`.
- https://github.com/eclipse-sirius/sirius-web/issues/6815[#6815] [tree] Add `ITreeItemPaletteProvider` interface in order to provide a tree item palette at once
- https://github.com/eclipse-sirius/sirius-web/issues/6826[#6826] [tree] Use the `palette` query on `TreeDescription` to render the tree item palette.
- https://github.com/eclipse-sirius/sirius-web/issues/6829[#6829] [diagram] [tree] Add the ability to customize the palette computed in a diagram or tree thanks to various post processors.
...
- https://github.com/eclipse-sirius/sirius-web/issues/6866[#6866] [tree] Add `labelExpression` to `CustomTreeItemContextMenuEntry`.
...
- [cleanup] Add formatting related tasks to package.json to make them easier to use.
...
- [releng] Start enforcing the size limit check for pull requests
- https://github.com/eclipse-sirius/sirius-web/issues/6691[#6691] [diagram] Add a top margin for child node with free form layout
- https://github.com/eclipse-sirius/sirius-web/issues/6705[#6705] Use backend internationalization for `ITreeItemContextMenuEntryProvider` contributions rendered in the tree palette.
- https://github.com/eclipse-sirius/sirius-web/issues/6695[#6695] [project] Expose previous project name in 'ProjectNameUpdatedEvent'
- https://github.com/eclipse-sirius/sirius-web/issues/6635[#6635] [sirius-web] Add a dedicated package for Markdown edition and rendering.
...
- https://github.com/eclipse-sirius/sirius-web/issues/6461[#6461] [core] Make `IOmniboxCommandOrderer` extensible by downstream applications.
...
- https://github.com/eclipse-sirius/sirius-web/issues/6763[#6763] [sirius-web] Allow project semantic data to be renamed.
- https://github.com/eclipse-sirius/sirius-web/issues/6773[#6773] [tree] Extract tree filters logic into a dedicated hook.
- https://github.com/eclipse-sirius/sirius-web/issues/6734[#6734] [sirius-web] Allow downstream applications to configure the static locations to use to load frontend artifacts.
...
- https://github.com/eclipse-sirius/sirius-web/issues/6790[#6790] [tree] Extract tree states into a dedicated hook.
- https://github.com/eclipse-sirius/sirius-web/issues/6786[#6786] [diagram] Add pin/unpin to group palette.
- https://github.com/eclipse-sirius/sirius-web/issues/6803[#6803] [tree] Extract explorer selection logic into a dedicated hook.
- https://github.com/eclipse-sirius/sirius-web/issues/6779[#6779] [diagram] Add the `diagramServices.deleteViews(nodes)` AQL service to let studio makers configure tools that remove multiple nodes from a diagram without deleting their semantic elements, and use it in the Flow unsynchronized diagram's group palette.
- https://github.com/eclipse-sirius/sirius-web/issues/6194[#6194] [core] Ensure that the frontend GraphQL error payloads now use `ErrorPayload#messages` instead of `ErrorPayload#message`, and the affected mutation hooks now log server feedback with `useMultiToast` when they use custom payloads and `useReporting` when they use the shared success/error contract.
- https://github.com/eclipse-sirius/sirius-web/issues/6047[#6047] [deck] Simplify the lifecycle of the Deck representation event processor.
Add dedicated executors and fluent assertion classes for the deck card creation, deletion, visibility, and lane-collapse mutations.
- https://github.com/eclipse-sirius/sirius-web/issues/6844[#6844] [diagram] Expose available variables in the Javadoc of generated View DSL builders.
...
- [ADR-231] Add support for decorating events with metadata
- [ADR-232] Add support for GitHub Flavored Markdown rendering
- [ADR-233] Use the diagram palette for tree based representations
...
- https://github.com/eclipse-sirius/sirius-web/issues/6055[#6055] [releng] Sirius Web now requires **Java 21** or later to build *and* to run.
- https://github.com/eclipse-sirius/sirius-web/issues/6400[#6400] [gantt] Add a new `dependencyLink` concept to improve dependency management in Gantt. The Gantt descriptions have been changed.
- https://github.com/eclipse-sirius/sirius-web/issues/6126[#6126] [diagram] The extension point `diagramPalette#tool` has been removed in favor of `palette#quickTool`. This new extension point now supports quick tools formultiple elements selected at once.
- The `variables.css` file, which used to define custom colors an typography settings has been removed.
It has long been unused by Sirius Web itself (since the transition to MUI).
- https://github.com/eclipse-sirius/sirius-web/issues/5928[#5928] [workbench] Each view displayed in the left or right side panel is now responsible from its own title bar.
- https://github.com/eclipse-sirius/sirius-web/issues/6587[#6587] [diagram] Added `IEditingContext` parameter to `IPaletteToolsProvider` methods
- https://github.com/eclipse-sirius/sirius-web/issues/6595[#6595] [diagram] `appliesToDiagramRoot` field was removed from `isSingleClickOnTwoDiagramElementsTool`.
- https://github.com/eclipse-sirius/sirius-web/issues/1361[#1361] [diagram] The Graphql API `deleteFromDiagram` has been removed.
- https://github.com/eclipse-sirius/sirius-web/issues/6546[#6546] [diagram] The `Palette` component and its related types are now exported from the palette package
...
- https://github.com/eclipse-sirius/sirius-web/issues/6583[#6583] [core] Causality chains starting from `InvokeSingleClickOnDiagramElementToolInput` now contain an additional `DecoratedCause` caused by the original input.


=== Dependency update

- https://github.com/eclipse-sirius/sirius-web/issues/6055[#6055] [releng] As of version 2026.7.0, Sirius Web now requires **Java 21** or later.
- [releng] Switch to https://github.com/spring-projects/spring-boot/releases/tag/v4.0.7[Spring Boot 4.0.7]
- [releng] Switch to https://github.com/vitejs/vite/releases/tag/v8.0.14[Vite 8.0.14] and https://github.com/vitejs/vite-plugin-react/releases/tag/plugin-react%406.0.2[@vitejs/plugin-react 6.0.2]
- [releng] Switch to `@fontsource/roboto` v5.2.10.
An issue was present in the way fonts were declared in Sirius Web.
One one side, some legacy code had a dependency to Lato which has never been used since the switch to MUI years ago (this has been removed).
On another side, we had a MUI theme declaring that the font family was `Roboto, Helvetica, Arial, sans-serif` (the default MUI behavior).
And finally, we had a dependency to `Roboto` which was not used at all.
As a result, those who had `Roboto` installed on their machine could view Sirius Web using `Roboto`, otherwise it would fall back to `Helvetica`, `Arial` or some basic `sans-serif` fonts.
This could massively change the appearance of Sirius Web depending on the machine.
The situation is now clear, our theme is using `Roboto` so we are loading `Roboto` and everybody should see Sirius Web the same way.
In the case of Sirius Web, we now have import declarations such as `import '@fontsource/roboto/300.css';` for the `300`, `400`, `500` and `700` font weights in our `index.tsx` given that our MUI theme is using Roboto with those specific font weights.
The font weights used in the default MUI theme can be found here[https://mui.com/material-ui/customization/default-theme].
- https://github.com/eclipse-sirius/sirius-web/issues/5928[#5928] [releng] Switch to React Resizable Panels 4.11.2.
This switch enables us to use other units such as `px` or `rem` instead of `%` to specify panels' size.
It also enables us to disable panel resize for a better control when needed.
- [releng] Switch `@ObeoNetwork/gantt-task-react` to `0.6.6`
- [releng] Switch to EMFJson 2.5.4
- [releng] Switch to https://download.eclipse.org/modeling/emf/emf/builds/release/2.46.0/index.html[Eclipse EMF 2.46.0]
* `org.eclipse.emf.common` from `2.31.0` to `2.45.0`
* `org.eclipse.emf.ecore` from `2.37.0` to `2.42.0`
* `org.eclipse.emf.edit` from `2.22.0` to `2.23.0`
* `org.eclipse.emf.ecore.xmi` from `2.38.0` to `2.40.0`
* `org.eclipse.emf.codegen.ecore` from `2.39.0` to `2.46.0`


=== Bug fixes

- https://github.com/eclipse-sirius/sirius-web/issues/5301[#5301] [core] Fix browser history on workbenches
- https://github.com/eclipse-sirius/sirius-web/issues/6557[#6557] [sirius-web] The workbench now respects the order of the left and right views provided in the `WorkbenchConfiguration`.
- https://github.com/eclipse-sirius/sirius-web/issues/6528[#6528] [diagram] Restore reset label position and size functionality for edge-on-edge labels
- [releng] Update the versions of Express, Puppeteer and Chrome to more recent compatible ones in the `diagram-image-server` application.
- https://github.com/eclipse-sirius/sirius-web/issues/6574[#6574] [diagram] Prevent arrange all action from failing when an edge exists between a parent node and its child.
- https://github.com/eclipse-sirius/sirius-web/issues/6599[#6599] [diagram] Remove role `button` on the nodes computed during internal layout.
- https://github.com/eclipse-sirius/sirius-web/issues/6593[#6593] [diagram] Change direct edit background color to white instead of transparent.
This is to make it easier to type when the area overlaps another elements.
- https://github.com/eclipse-sirius/sirius-web/issues/6610[#6610] [diagram] Prevent edge reset when source and target nodes are vertically or horizontally aligned
- https://github.com/eclipse-sirius/sirius-web/issues/6622[#6622] [diagram] Fix a bug when re-invoking the last tool from the shortcut in the palette when the tool is overridden using `paletteToolOverrideExtensionPoint`, the `PaletteToolOverriddenContributionComponentProps` now has a `onInvoked` prop to call in order to set the last tool used.
- https://github.com/eclipse-sirius/sirius-web/issues/6623[#6623] [diagram] Fix highlighting of matching tool's label when using search in the palette.
- https://github.com/eclipse-sirius/sirius-web/issues/6652[#6652] [diagram] Fix fork representation contribution not rendering.
- https://github.com/eclipse-sirius/sirius-web/issues/6628[#6628] [diagram] Prevent diagram tools from processing utility nodes
- https://github.com/eclipse-sirius/sirius-web/issues/6657[#6657] [diagram] Fix last tool used for tree palette overridden contributions.
- https://github.com/eclipse-sirius/sirius-web/issues/6639[#6639] [diagram] Fix an issue where the palette had a sliding effect when first opened.
- https://github.com/eclipse-sirius/sirius-web/issues/6646[#6646] [diagram] Fix an issue where connector tools where rendered on a node palette.
- https://github.com/eclipse-sirius/sirius-web/issues/6627[#6627] [diagram] Prevent pinned nodes from moving during arrange all action
- https://github.com/eclipse-sirius/sirius-web/issues/6656[#6656] Align menu icon with the project title in the navbar.
- https://github.com/eclipse-sirius/sirius-web/issues/6680[#6680] [sirius-web] Sort project semantic data by creation date
- https://github.com/eclipse-sirius/sirius-web/issues/6675[#6675] [sirius-web] Fix failure when decoding fonts in production.
- https://github.com/eclipse-sirius/sirius-web/issues/6554[#6554] [diagram] Fix last tool used for palette tools contributions
...
- https://github.com/eclipse-sirius/sirius-web/issues/6552[#6552] [diagram] Add `paletteToolExtensionPoint` to contribute tools on a palette, you can contribute tools on a specific tool section.
...
- https://github.com/eclipse-sirius/sirius-web/issues/6583[#6583] [core] Add the ability to decorate causes with metadata.
The `DecoratedCause` record can now be used to wrap an existing cause and provide a label with additional information on what happened while processing the event.
An initial implementation has been done for `InvokeSingleClickOnDiagramElementToolInput`, providing a label containing the name of the executed tool, the label of the selected node(s)/edge(s), as well as the name of their descriptions.
The resulting label looks like "Used 'Create Component' on the Component Diagram business architecture".
This feature allows to share information that is easy to collect at one point (e.g. in the tool handler) and hard to retrieve down the event chain (e.g. in an event listener reacting to the handler's change, where only the `toolId` and `diagramElementIds` are available).


=== Improvements

- [releng] Fix the diagram image server Dockerfile
...
- https://github.com/eclipse-sirius/sirius-web/issues/6559[#6559] [workbench] Add `children` prop support for navigation bar contributions
- https://github.com/eclipse-sirius/sirius-web/issues/6400[#6400] [gantt] Introduce the concept of dependencyLink, which may be used to extends dependency management for the Gantt tasks
- https://github.com/eclipse-sirius/sirius-web/issues/6516[#6516] [releng] The Sirius Web build now generates and publishes SBOMs on https://sbom.eclipse.org/projects/9654724d-97bf-4f9b-84be-d8ea7884f73e[the Eclipse Foundation's DependencyTrack instance].
- https://github.com/eclipse-sirius/sirius-web/issues/6529[#6529] [diagram] Use the diagram representation name for exported image filenames
- https://github.com/eclipse-sirius/sirius-web/issues/6123[#6123] [diagram] Improve `PaletteToolList` in order to preparer for the contribution of tool sections with extensions point.
- https://github.com/eclipse-sirius/sirius-web/issues/5928[#5928] [sirius-web] Move responsibility of view headers directly inside the view component instead of `WorkbenchPart`.
- https://github.com/eclipse-sirius/sirius-web/issues/5892[#5892] [sirius-web] Make `ISearchService` extensible by downstream applications.
Downstream applications can implement `ISearchServiceDelegate` to define their own searching strategy.
- https://github.com/eclipse-sirius/sirius-web/issues/6578[#6578] [test] Make test runner GraphQL queries public.
- https://github.com/eclipse-sirius/sirius-web/issues/1361[#1361] [diagram] Remove hard-coded diagram delete tool
- https://github.com/eclipse-sirius/sirius-web/issues/6542[#6542] [diagram] Extract the draggable feature from the palette in order to facilitate the export of the Palette component
- https://github.com/eclipse-sirius/sirius-web/issues/2928[#2928] [diagram] Render creation handles in a dedicated node, this ensure that the handles are always clickable event if an edge is passing in front of them.
- https://github.com/eclipse-sirius/sirius-web/issues/6612[#6612] [core] `index.html` can now be processed through `IIndexProcessor` before being sent to the client.
- https://github.com/eclipse-sirius/sirius-web/issues/6605[#6605] [diagram] If multiple `IPaletteToolsProvider` to tool sections with the same `id`, all their tools are merged into a single section.
The label and icon of the resulting tool section are the ones from the first `IPaletteToolsProvider` encountered.
Inside the resulting section, the tools are ordered depending on the order the `IPaletteToolsProvider` are encountered.
Added `IDiagramPaletteCustomizer` to customize all contributed tool section from `IPaletteToolsProvider`.
- https://github.com/eclipse-sirius/sirius-web/issues/6625[#6625] [sirius-web] Make the project title more compact
- https://github.com/eclipse-sirius/sirius-web/issues/6792[#6792] [diagram] Increase ReactFlow's node drag threshold from 1 px to 5 px to reduce unintended node dragging.
- https://github.com/eclipse-sirius/sirius-web/issues/6795[#6795] [diagram] Allow users to start a lasso selection on a node without selecting that node or its containers, so nested nodes can be selected while their containers remain unselected.
...
- [releng] Switch to https://github.com/xyflow/xyflow/releases/tag/@xyflow/react@12.10.2[xyflow 12.10.2]
